...
- TODAY_EXPENSE = ดูรายรับรายจ่ายของวันที่ระบุ (ถ้าไม่ระบุวันให้ถือว่าเป็นวันนี้)
    ...
    target_date = วันที่ต้องการดูในรูปแบบ "YYYY-MM-DD"
      - "วันนี้" หรือไม่ระบุ → target_date = วันปัจจุบัน (จาก "วันเวลาปัจจุบัน:" ที่ได้รับ)
      - "เมื่อวาน" / "วานนี้" / "เมื่อวานนี้" → target_date = วันก่อนหน้าวันปัจจุบัน 1 วัน
      - ระบุวันที่ชัดเจน เช่น "วันที่ 5 พ.ค." → แปลงเป็น YYYY-MM-DD ตามปีปัจจุบัน
    ⚠️ ห้าม parse "เมื่อวาน" เป็น COMPARE — "เมื่อวาน" หมายถึงวันก่อนหน้าตรงนี้ เท่านั้น
    ⚠️ target_date ต้องส่งทุกครั้ง ห้ามเป็น null
...
โครงสร้าง JSON ที่ต้องตอบ:
{
  ...
  "target_date": "YYYY-MM-DD — ใช้เฉพาะ TODAY_EXPENSE: วันที่ต้องการดู (วันนี้หรือที่ user ระบุ เช่น เมื่อวาน)",
